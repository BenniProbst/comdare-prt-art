...
public:
    ...
    [[nodiscard]] PoolDescriptor&       get(PoolKind k) noexcept { return descriptors_[index(k)]; }
    [[nodiscard]] PoolDescriptor const& get(PoolKind k) const noexcept { return descriptors_[index(k)]; }
    ...
    void note_alloc(PoolKind k, std::size_t bytes) noexcept { get(k).stats.on_alloc(bytes); }
    void note_dealloc(PoolKind k, std::size_t bytes) noexcept { get(k).stats.on_dealloc(bytes); }
    ...
    [[nodiscard]] static constexpr std::size_t index(PoolKind k) noexcept { return static_cast<std::size_t>(k); }

    [[nodiscard]] std::array<PoolDescriptor, kPoolCount> const& all() const noexcept { return descriptors_; }
