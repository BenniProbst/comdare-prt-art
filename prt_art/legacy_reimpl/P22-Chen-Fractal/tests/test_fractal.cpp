...
TEST(DiskPage, InnerNodeFitsInCacheLine) { EXPECT_LE(sizeof(fr::InnerCacheNode), fr::kCacheLineBytes); }
...
TEST(DiskPage, AllocateInnerNodesUntilFull) {
    ...
    for (std::size_t i = 1; i < fr::kInnerNodesPerPage; ++i) { EXPECT_GE(page.allocate_inner_node(), 0); }
