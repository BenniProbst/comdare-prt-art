# P16-Bender-TreeLayout — Re-Implementation-Skelett

**Paper:** Tree Layout in Multilevel Memory
**Autor/Jahr:** Bender/Demaine/Farach-Colton 2002
**Kernkonzept:** Probability-driven Layout (Greedy O(N log B)) ueber Access-Verteilung
**Concept-Anbindung:** `MemoryLayout` (Achse: Memory-Layout)
...
1. **Pseudocode-Extraktion** aus Originalpaper (siehe `Forschungsarbeiten/Tree Layout in Multilevel Memory (BenderDemaineFarach-Colton 2002).pdf`)
2. **Concept-Anbindung:** Implementiert das `MemoryLayout`-Concept
   (siehe `search_engine/memory_layout/`)
3. **C++23-Implementation** mit modernem Constexpr / Concepts / Modules
4. **Adapter** an die Bausteine-Matrix-Achse "Memory-Layout"
...
├── include/
│   └── probability_layout.hpp             (Concept-Anbindung)
├── src/
│   └── probability_layout.cpp             (Implementation)
└── tests/
    └── probability_layout_test.cpp        (Google-Tests)
```
