...
static_assert(sizeof(CssNodePage<7>) <= kCacheLineBytes, "CssNodePage<7> must fit into a 64-byte cache line");
...
public:
    ...
    [[nodiscard]] static constexpr std::size_t child_index(std::size_t parent_idx, std::size_t slot) noexcept {
        return parent_idx * (Fanout + 1) + slot + 1;
    }

    [[nodiscard]] static constexpr std::size_t parent_index(std::size_t child_idx) noexcept {
