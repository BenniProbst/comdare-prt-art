...
struct CostModel {
    ...
    [[nodiscard]] double total(std::uint64_t i, std::uint64_t m, std::uint64_t b, std::uint64_t t) const noexcept {
        return weight_instructions * static_cast<double>(i) + weight_memory_stalls * static_cast<double>(m) +
               weight_branch * static_cast<double>(b) + weight_tlb * static_cast<double>(t);
    }
};
