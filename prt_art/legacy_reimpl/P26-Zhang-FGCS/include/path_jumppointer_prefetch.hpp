...
enum class PrefetchKind : std::uint8_t { Path = 0, JumpPointer = 1 };
...
public:
    void record_read(std::uint64_t block_id) { ++stats_[block_id].read_count; }
    ...
    [[nodiscard]] PrefetchKind recommend(std::uint64_t block_id) const {
        ...
        double const        path_rate =
            path_total > 0 ? static_cast<double>(s.path_hits) / static_cast<double>(path_total) : 0.0;
        double const jump_rate =
            jump_total > 0 ? static_cast<double>(s.jump_hits) / static_cast<double>(jump_total) : 0.0;
