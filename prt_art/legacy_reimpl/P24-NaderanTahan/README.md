# P24-NaderanTahan — Re-Implementation-Skelett

**Paper:** Why Does Data Prefetching Not Work for Modern Workloads
**Autor/Jahr:** Naderan-Tahan/Sarbazi-Azad 2016
**Kernkonzept:** Studie/Analyse: Useless Prefetches dominieren bei modernen Workloads
**Concept-Anbindung:** `Telemetry` (Achse: Measurement)
...
1. **Pseudocode-Extraktion** aus Originalpaper (siehe `Forschungsarbeiten/WhyDoes Data Prefetching Not Work for Modern Workloads.pdf`)
2. **Concept-Anbindung:** Implementiert das `Telemetry`-Concept
   (siehe `search_engine/memory_layout/`)
3. **C++23-Implementation** mit modernem Constexpr / Concepts / Modules
4. **Adapter** an die Bausteine-Matrix-Achse "Measurement"
...
├── include/
│   └── useless_prefetch_study.hpp             (Concept-Anbindung)
├── src/
│   └── useless_prefetch_study.cpp             (Implementation)
└── tests/
    └── useless_prefetch_study_test.cpp        (Google-Tests)
```
