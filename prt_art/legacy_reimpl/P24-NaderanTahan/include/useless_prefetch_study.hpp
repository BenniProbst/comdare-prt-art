...
struct UsefulnessMetrics {
    ...
    [[nodiscard]] double usefulness_ratio() const noexcept {
        return total_prefetches > 0 ? static_cast<double>(useful_prefetches) / static_cast<double>(total_prefetches)
                                    : 0.0;
    }
    [[nodiscard]] double waste_ratio() const noexcept {
        return total_prefetches > 0 ? static_cast<double>(useless_prefetches) / static_cast<double>(total_prefetches)
                                    : 0.0;
    }
};
...
public:
    ...
    void evict(std::uint64_t addr) {
        ...
        if (!it->second) { ++metrics_.useless_prefetches; }
        pending_.erase(it);
    }
